...
const schema = Joi.object({
  ...
  tenantCpfOrEmail: Joi.string().required(),
...
module.exports = async (req, res, next) => {
  ...
  const tenantExists = await customerService.getByCpfOrEmail(req.body.tenantCpfOrEmail, 'tenant');
  if (!tenantExists) {
    return next({
      statusCode: StatusCodes.NOT_FOUND,
  ...
  if (tenantExists.cpf === locatorExists.cpf) {
    return next({
      statusCode: StatusCodes.NOT_FOUND,
      message: 'Locator and tenant are the same person'
    });
  };
